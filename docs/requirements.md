...
| LST-SCP-003 | Native Alexa-list synchronisation, public certification, a Lists-owned UI, multi-household tenancy, LLM matching, purchasing, scanning, meal planning, and inventory management are out of scope for version 1. | Repository review; README limitations; no contradicting infrastructure or runtime path.                                           |
| LST-SCP-004 | Provide named task lists as sections inside the configured Todoist project without exposing unrelated projects; preserve legacy shopping routes.                                                                | Task-list service/adapter/controller tests; OpenAPI; ADR 0004; real provider smoke pending.                                       |
