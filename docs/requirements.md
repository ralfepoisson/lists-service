...
| LST-SCP-002 | Todoist is the authoritative datastore and no separate application database is introduced. Shopping and Task Lists resolve the acting account through the protected tenant catalogue.                                            | Repository adapters and configuration; ADRs 0002, 0005, and 0006; real provider evidence.                                         |
| LST-SCP-003 | Native Alexa-list synchronisation, public certification, a Lists-owned UI, browser Todoist onboarding/disconnection, LLM matching, purchasing, scanning, meal planning, and inventory management are out of scope for version 1. | Repository review; README limitations; no contradicting runtime path.                                                             |
| LST-SCP-004 | Superseded by ADR 0005: the earlier section-inside-one-project model is retained only as historical design context and is not the Task Lists runtime model.                                                                      | ADRs 0004 and 0005; repository and contract review.                                                                               |
| LST-SCP-005 | Support scoped task create, edit, remove, complete, and exact-permutation reorder; deleting a list completes active tasks then archives it.                                                                                      | Application/adapter/REST tests; OpenAPI; real provider archive-history verification pending.                                      |
| LST-SCP-006 | Represent each Todoist project visible through the acting tenant's connected token as a Task List; Shopping/Alexa/automation use a deployment-bound account in the same catalogue.                                               | Tenant catalogue, repository and controller tests; ADRs 0005 and 0006; real multitenant provider acceptance pending.              |
| LST-SCP-007 | Expose bounded workspace search over the acting tenant's visible task-list names and active task content using normalized, allowlisted navigation targets.                                                                       | Application/controller tests; OpenAPI; real multitenant provider acceptance pending.                                              |
...
| LST-FUN-011 | Generate a printable PDF snapshot from the current active Todoist items without persisting a duplicate list or generated document.                                         | `GET /v1/items.pdf` OpenAPI contract; print-service and REST adapter tests; PDF structure/content extraction. |
...
| LST-API-002 | The internal `POST /api/v1/search` provider route uses the shared cross-service `{items}` response contract, bounded inputs, private no-store caching, and no caller-controlled tenant authority.                                                         | OpenAPI; REST controller tests; BFF contract tests.                          |
| LST-SEC-001 | All REST routes except `/health` require authentication. Shopping accepts a verified Life2 principal or deployment-bound automation principal; Task Lists and connection routes require a verified Life2 JWT.                                             | Auth strategy/controller tests; deployed negative and positive calls.        |
...
| LST-SEC-007 | Life2 JWT verification pins HS256, issuer `life2.ralfe.me`, audience `account`, time and identity claims. Its non-empty verified `accountId` exclusively selects a server-side tenant catalogue entry; callers cannot supply or override tenant identity. | JWT/catalogue/controller isolation tests and deployed two-tenant acceptance. |
| LST-SEC-008 | The tenant catalogue contains only unique `accountId` to token-secret references. Each Todoist token is a separate protected secret; malformed/duplicate catalogues fail closed and no credential is exposed in status or logs.                           | Catalogue tests, secret/IAM review, negative runtime evidence.               |
...
| LST-TOD-004 | Require each catalogue entry to specify `shoppingProjectId` preferentially or `shoppingProjectName`; for a name, initialize once by reusing one exact match or creating it, refuse ambiguity, and never scan on every item operation. | Catalogue/provisioner tests; deployed configuration and real-provider evidence.     |
| LST-TOD-005 | For Task Lists, list/create/archive Todoist projects and scope every task operation to the requested project using the tenant token selected by verified `accountId`; refuse Inbox archival.                                          | Repository/controller tests; ADR 0005; real tenant-isolation and provider evidence. |
| LST-TOD-006 | Expose credential-free connection status with `canManageConnection=false`; reject browser authorization start and disconnect in this release.                                                                                         | REST/OpenAPI tests and deployed negative calls.                                     |
