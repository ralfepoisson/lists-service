...
| LST-TOD-004 | Support `TODOIST_PROJECT_ID` preferentially or resolve `TODOIST_PROJECT_NAME` once through a startup/deployment utility and persist the ID; never scan all projects on every request.                                   | Configuration/resolver code and tests; deployed configuration evidence.           |
