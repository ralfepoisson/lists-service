...
| LST-SCP-002 | Todoist is the authoritative datastore and no separate application database is introduced. ADR 0005 supersedes the one-project restriction for Task Lists; the configured project remains the legacy Shopping/Alexa boundary.    | Repository adapters and configuration; ADRs 0002 and 0005; real provider evidence.                                                |
...
| LST-SCP-006 | Represent each Todoist project visible through the acting tenant's connected token as a Task List, while preserving the separate legacy Shopping/Alexa owner token and configured project.                                       | Tenant catalogue, repository and controller tests; ADR 0005; real multitenant provider acceptance pending.                        |
...
| LST-SEC-001 | All REST routes except `/health` require authentication. Task Lists and connection routes require a verified Life2 JWT and reject static automation authentication; the static token remains legacy-Shopping-only.                                        | Auth strategy/controller tests; deployed negative and positive calls.        |
